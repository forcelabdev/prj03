...
// Rulet helper functions
const RED_NUMBERS = new Set([1, 3, 5, 7, 9, 12, 14, 16, 18, 19, 21, 23, 25, 27, 30, 32, 34, 36])

function getRouletteNumberStyle(num: number): { bg: string; text: string } {
  if (num === 0) return { bg: "#16a34a", text: "#fff" }
  if (RED_NUMBERS.has(num)) return { bg: "#dc2626", text: "#fff" }
  return { bg: "#1a1a1a", text: "#fff" }
}

function isRouletteGame(game: Game): boolean {
  const checkStr = [game.name, game.id, game.game_code, game.gameCode, game.game_name]
    .filter(Boolean)
    .join(" ")
    .toLowerCase()
  return checkStr.includes("rulet") || checkStr.includes("roulette")
}

function isBlackjackGame(game: Game): boolean {
  const checkStr = [game.name, game.id, game.game_code, game.gameCode, game.game_name]
    .filter(Boolean)
    .join(" ")
    .toLowerCase()
  return checkStr.includes("blackjack") || checkStr.includes("black_jack")
}

function generateRouletteNumbers(count: number): number[] {
  return Array.from({ length: count }, () => Math.floor(Math.random() * 37))
}

function generateSeats(total = 7): boolean[] {
  return Array.from({ length: total }, () => Math.random() > 0.45)
}
...
function RouletteNumbers({ gameId, betRange }: { gameId: string; betRange?: string }) {
  const [numbers, setNumbers] = useState<number[]>(() => generateRouletteNumbers(5))

  useEffect(() => {
    const interval = setInterval(() => {
      setNumbers((prev) => {
        const newNum = Math.floor(Math.random() * 37)
        return [newNum, ...prev.slice(0, 4)]
      })
    }, 7000 + (gameId.charCodeAt(0) % 5) * 600)
  ...
  return (
    <div
      ...
      style={{ background: "linear-gradient(to top, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.6) 70%, transparent 100%)" }}
    >
      <div className="flex items-center gap-2 mb-1">
        {numbers.map((num, i) => {
          const style = getRouletteNumberStyle(num)
          if (i === 0) {
            return (
              <span
                key={i}
                className="inline-flex items-center justify-center font-bold shrink-0 lg:!text-sm"
                style={{
          ...
          const textColor = num === 0 ? "#22c55e" : RED_NUMBERS.has(num) ? "#ef4444" : "#ffffff"
          return (
            <span
              key={i}
              className="font-bold lg:!text-sm"
              style={{ color: textColor, fontSize: "12px", fontWeight: 700 }}
            >
              {num}
            </span>
          )
      ...
      <p className="font-semibold mt-0.5" style={{ fontSize: "16px", color: "#ffffff" }}>
        ₺ {betRange || "2-200.000"}
      </p>
    </div>
  )
...
function SeatIcon({ occupied, size = 13.5 }: { occupied: boolean; size?: number }) {
  if (occupied) {
    return (
      <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 13 14" fill="#ffffff">
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 13 14" fill="none" stroke="#ffffff" strokeWidth="0.7">
      <path d="M11.58 6c-1.17 0-2.13.95-2.13 2.12v.4a8.82 8.82 0 00-2.92-.4c-1.54 0-2.45.22-2.92.4v-.4A2.136 2.136 0 001.48 6a1.33 1.33 0 00-.5 2.55c.14.07.24.21.24.37l.26 1.06c0 .96.73 1.75 1.65 1.85a.65.65 0 00.28 1.23.65.65 0 00.3-1.22h5.64a.65.65 0 00.3 1.22.65.65 0 00.28-1.23c.93-.1 1.65-.9 1.65-1.85l.27-1.06c0-.16.1-.3.24-.37A1.33 1.33 0 0011.58 6z" />
      <path d="M4.34 7.5c.55-.1 1.27-.17 2.2-.17.91 0 1.63.07 2.18.17.21-.96.9-1.74 1.8-2.1V4a3.99 3.99 0 00-7.97 0v1.4c.9.36 1.58 1.14 1.79 2.1z" />
    </svg>
  )
...
function BlackjackSeats({ gameId, betRange }: { gameId: string; betRange?: string }) {
  const [seats, setSeats] = useState<boolean[]>(() => generateSeats())
  ...
  useEffect(() => {
    ...
    }, 6000 + (gameId.charCodeAt(0) % 6) * 700)
    return () => clearInterval(interval)
  }, [gameId])
      ...
      <div className="flex items-center gap-1 mb-1">
        {seats.map((isOccupied, i) => (
          <span key={i} className="inline-flex">
            <span className="lg:hidden"><SeatIcon occupied={isOccupied} size={13.5} /></span>
            <span className="hidden lg:inline-flex"><SeatIcon occupied={isOccupied} size={19} /></span>
          </span>
        ))}
      </div>
      <p className="font-semibold mt-0.5 text-sm lg:text-base" style={{ color: "#ffffff" }}>
        ₺ {betRange || "50-25.000"}
      </p>
    </div>
  )
...
function LiveGameCard({ game, onClick }: { game: Game; onClick: () => void }) {
  const showRoulette = isRouletteGame(game)
  const showBlackjack = !showRoulette && isBlackjackGame(game)
  const gameKey = game.id || game.game_code || game.gameCode || game.name || "game"

  return (
    <div className="flex flex-col cursor-pointer" onClick={onClick}>
      <div className="relative rounded-lg lg:rounded-none overflow-hidden bg-zinc-800 w-full h-[124px] max-w-[193px] lg:max-w-[188px] lg:w-[188px] group">
        {game.image && <img src={game.image} alt={game.name} className="absolute inset-0 w-full h-full object-cover transition-[filter] duration-250 group-hover:blur-[4px]" />}
        ...
        {showRoulette ? (
          <RouletteNumbers gameId={gameKey} betRange={game.betRange} />
        ) : showBlackjack ? (
          <BlackjackSeats gameId={gameKey} betRange={game.betRange} />
        ) : game.betRange ? (
          <div className="absolute bottom-2 left-2">
            <span className="text-white text-[11px] font-medium">{game.betRange}</span>
          </div>
        ) : null}
      </div>
      ...
      <div className="lg:hidden mb-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mt-4">
          {mobileVisibleGames.map((game, index) => (
            <LiveGameCard key={game.id || index} game={game} onClick={() => onGameClick(game)} />
          ))}
        </div>
        {hasMoreMobile && (
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (categoriesRes.success && categoriesRes.categories) {
          ...
          const reversedCats = [...nonEmpty].reverse()
          setCategories(reversedCats)
          setCategoryTabs(["Lobi", ...reversedCats.map((c) => c.name)])
