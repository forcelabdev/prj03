...
//
// Bu dosyada useAuth, launchGame, launchStartedRef YOKTUR.
// Spin donma ve HMR crash bu sayede olmaz.
//
// Calisma mantigi:
//   1. Casino/modal'da oyuna tiklanir → launchGame() cagirilir
//   2. launchUrl gelir → router.push('/game/CODE?url=LAUNCH_URL')
//   3. Bu sayfa sadece ?url= parametresini okur, iframe'e verir
//   4. Spin istekleri dogrudan backend'e gider — Next.js karismaz
// ============================================================
...
function GameContent({ params }: { params: Promise<{ gameCode: string }> }) {
  ...
  // ADIM 1: URL parametrelerini oku — auth yok, launch yok
  useEffect(() => {
  ...
  // Tam ekran iframe — header/footer yok, auth yok, launch yok
  return (
    <iframe
      src={gameUrl}
      style={{
